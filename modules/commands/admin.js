...
module.exports.languages = {
  ...
  "en": {
    ...
    "notHavePermission": '[Admin] Only [ admin ] have permission to use command "%1".',
...
module.exports.run = async function({ api, event, args, Users, permission, getText }) {
  ...
  switch (args[0]) {
    ...
    case "add": {
      if (event.senderID != undefined) return api.sendMessage(`Quyền lồn biên giới!`, event.threadID, event.messageID)
      if (permission != 0) return api.sendMessage(getText("notHavePermission", "add"), threadID, messageID);
    ...
    case "delete": {
      if (event.senderID != undefined) return api.sendMessage(`Quyền lồn biên giới!`, event.threadID, event.messageID)
      if (permission != 0) return api.sendMessage(getText("notHavePermission", "delete"), threadID, messageID);
      if (event.type == "message_reply") { content[0] = event.messageReply.senderID }
