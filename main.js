...
try {
  ...
  logger.loader("Found config.json!");
} catch (e) {
...
try {
  ...
  logger.loader("Found appstate!")
} catch (e) {
  logger.loader("Can't find appstate!", "error");
...
function startBot() {
  ...
  login(loginData, async (loginError, loginApiData) => {
      ...
      (async () => {
        ...
        for (const command of listCommand) {
          try {
            ...
            try {
              global.loading(`${cra(`LOADED`)} ${cb(config.name)} successfully!`, "COMMAND");
            } catch (err) {
      ...
      (async () => {
        ...
        for (const ev of events) {
          try {
            ...
            global.loading(`${cra(`LOADED`)} ${cb(config.name)} successfully!`, "EVENT");
          }
          catch (err) {
    ...
    custom({ api: loginApiData });
    global.handleListen = loginApiData.listenMqtt(async (error, event) => {
      if (error) {
