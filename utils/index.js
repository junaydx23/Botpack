...
module.exports.connect = function() {
  if (config.CONNECT_LOG === true) {
    ...
    setTimeout(() => {
      ...
      const message = `The AppState is empty or either not readable! \n\nTry attempting to login by using your Facebook Account Credential's.`;
        ...
        rl.question(`${co('[ LOGIN ]')}` + `${chalk.yellow("  ") + "\u0045\u006D\u0061\u0069\u006C\u003A "}`, (email) => {
          rl.question(`${co('[ LOGIN ]')}` + `${chalk.yellow("  ") + "\u0050\u0061\u0073\u0073\u0077\u006F\u0072\u0064\u003A "}`, (password) => {
            ...
            setTimeout(() => {
              ...
              process.stdout.clearLine();
              process.stdout.cursorTo(0);
              console.log(co('[ LOGIN ]'), chalk.yellow('  '), 'Process complete successfully!');
            }, 10000);
            ...
            connect({ email, password }, (err, api) => {
              if (err) {
                ...
                console.log(chalk.dim(`\nPlease try to 're-run' the project and enter your login credentials again!`))
                console.log = noop;
                return;
              }
              ...
              console.log = logA;
              console.log(co('\r[ LOGIN ]'), chalk.yellow(''), `${greens('Account')} login successfully!`);
              rl.close();
              const fig = JSON.parse(fs.readFileSync(confg, 'utf8'));
              fig.CONNECT_LOG = true;
              fs.writeFileSync(confg, JSON.stringify(fig, null, 2), 'utf8');
              console.log(co('\r[ LOGIN ]'), chalk.yellow(''), `Please ${greens('Re-run')} the project to continue logging in.`);
              const message = '\n';
              let v = 0;
              const interval = 150;
