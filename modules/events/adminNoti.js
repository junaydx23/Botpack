    ...
    switch (logMessageType) {
      case "log:thread-admins": {
        if (logMessageData.ADMIN_EVENT === "add_admin") {
          ...
          api.sendMessage(`[ GROUP UPDATE ]\n❯ Added user ${logMessageData.TARGET_ID} as a group admin`, threadID);
        } else if (logMessageData.ADMIN_EVENT == "remove_admin") {
          dataThread.adminIDs = dataThread.adminIDs.filter(item => item.id !== logMessageData.TARGET_ID);
          api.sendMessage(`[ GROUP UPDATE ]\n❯ Remove user ${logMessageData.TARGET_ID} as a group admin`, threadID);
        }
        break;
      }
      case "log:user-nickname": {
        ...
        if (participant_id && nickname) {
          dataThread.nicknames = dataThread.nicknames || "clear the nickname";
          dataThread.nicknames[participant_id] = nickname;
          const participantName = await Users.getNameUser(participant_id);
          const formattedNickname = nickname || "cleared the nickname";
