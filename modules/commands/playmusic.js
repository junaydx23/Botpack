...
module.exports.run = async ({ api, event, args }) => {
  ...
  if (data.length < 2) {
    api.sendMessage(`[ ! ] Input music title`, event.threadID);
  } else {
    data.shift();
    ...
    const file = fs.createWriteStream(`${__dirname}/cache/audio_${senderID}.mp3`);
    ...
    async function main() {
      ...
      api.sendMessage({
        body: messageBody,
        attachment: fs.createReadStream(`${__dirname}/cache/audio_${senderID}.mp3`)
      }, event.threadID, event.messageID);
      }, 5000)

      // Remove video file after a delay of 5 seconds
      setTimeout(() => {
        fs.unlinkSync(`${__dirname}/cache/audio_${senderID}.mp3`);
