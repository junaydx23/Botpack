module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  return async function({ event }) {
    ...
    const { allowInbox, PREFIX, ADMINBOT, DeveloperMode, adminOnly, keyAdminOnly, APPROVED, approval } = global.config;
    ...
    const notApproved = '[ APPROVAL MODE ] • The thread is not approved.';   
    const replyAD = '[ DEV MODE ] • The bot is currently running on developer mode.';
    ...
    if (command && (command.config.name.toLowerCase() === commandName.toLowerCase()) &&(!APPROVED.includes(threadID) && !ADMINBOT.includes(senderID) && approval)) {
      return api.sendMessage(aa, threadID, async (err, info) => {
            await new Promise(resolve => setTimeout(resolve, 5 * 1000));
            return api.unsendMessage(info.messageID);
          });
    }
    if (typeof body === 'string' && body.startsWith(PREFIX) && (!APPROVED.includes(threadID) && !ADMINBOT.includes(senderID) && approval)) {
    ...
    if (command && (command.config.name.toLowerCase() === commandName.toLowerCase()) && (!ADMINBOT.includes(senderID) && DeveloperMode && senderID !== api.getCurrentUserID())) {
      return api.sendMessage(bb, threadID, async (err, info) => {
            await new Promise(resolve => setTimeout(resolve, 5 * 1000));
            return api.unsendMessage(info.messageID);
        });
    }
    if (typeof body === 'string' && body.startsWith(PREFIX) && (!ADMINBOT.includes(senderID) && DeveloperMode && senderID !== api.getCurrentUserID())) {
      return api.sendMessage(replyAD, threadID, async (err, info) => {
            await new Promise(resolve => setTimeout(resolve, 5 * 1000));
            return api.unsendMessage(info.messageID);
        });
    }

    if (typeof body === 'string' && body.startsWith(PREFIX) && (userBanned.has(senderID) && threadBanned.has(threadID) && allowInbox == ![] && senderID == threadID)) {
    ...
    if (command && command.config) {
      if (typeof command.config.usePrefix === 'undefined') {
        api.sendMessage(global.getText("handleCommand", "usePrefix", command.config.name), event.threadID, event.messageID);
        return;
      }
    }
    ...
    else if (!ADMINBOT.includes(senderID) && find) permission = 2;
    if (command && command.config && command.config.hasPermission && command.config.hasPermission > permission) {
