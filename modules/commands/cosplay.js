...
"https://i.imgur.com/FhC4jXa.jpg",
   ...
   var callback = () => 
     setTimeout(function() {
  api.sendMessage({ body:`Here your cosplay photo. \n\nTotal of the available image: ${link.length}`, attachment: fs.createReadStream(__dirname + "/cache/cosplay/cosplay.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/cosplay/cosplay.jpg"));
     }, 5000);
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname + "/cache/cosplay/cosplay.jpg")).on("close",() => callback());
 };